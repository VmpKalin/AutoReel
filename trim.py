...
Usage: python3 trim.py input.mp4 15
    ...
    python3 trim.py input.mp4 15 --start 5   # from 5s to 20s
"""
...
def trim_video(input_path: str, duration: float, start: float = 0.0, output_path: str = None):
    ...
    print(f"✂️  Trimming: {start:.1f}s → {end:.1f}s ({duration:.1f}s)")
...
def main():
    parser = argparse.ArgumentParser(description="Trim video to a specified duration")
    ...
    parser.add_argument("-s", "--start", type=float, default=0.0,
                        help="Trim start time in seconds (default: 0)")
    parser.add_argument("-o", "--output", default=None,
                        help="Output file (default: input_trimmed.mp4)")
